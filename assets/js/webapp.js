...
function search_stations(relocate) {
  ...
  $.post('/search/traffic-items', search_params, function(raw_data) {
    ...
    //
    //storage['areas'].clearLayers();
    /*
    $(raw_data.response).each(function(i, item) {
      
      if (item.type)
        marker = L.marker([item.lat, item.lon], { icon: storage.traffic_item_type_icons[item.type], traffic_icon_id: item.id } );
      else
        marker = L.marker([item.lat, item.lon], { traffic_icon_id: item.id });
      marker.on('click', function (current_marker) {
        var marker_id = current_marker['target']['options']['traffic_icon_id'];
        $.getJSON('/api/traffic-item?id=' + marker_id, function(data) {
          data = data['response'];
      ...
      if (item['area']) {
        if (item['area']['type'] == 'Polygon') {
          area = L.polyline(item['area']['coordinates'], {'color': '#FF3333', 'opacity': 1, 'fill': true})
          storage['areas'].addLayer(area);
        }
        else if (item['area']['type'] == 'MultiPolygon') {
          area = L.multiPolyline(item['area']['coordinates'], {'color': '#FF3333', 'opacity': 1, 'fill': true})
          storage['areas'].addLayer(area);
        }
        else if (item['area']['type'] == 'LineString') {
          area = L.polyline(item['area']['coordinates'], {'color': '#FF3333', 'opacity': 0.5});
          storage['areas'].addLayer(area);
        }
      }
    });*/
